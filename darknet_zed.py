...
# Using open3D to process point cloud data
from open3d import *
...
def main(argv):
    ...
    configPath = darknet_path + "cfg/my_boat.cfg"
    weightPath = "yolov2-tiny-voc_final.weights"
    # weightPath = "my_yolov3-tiny.weights"
    metaPath = "myboat.data"
    svoPath = None
    ...
    # uncomment it to save images
    #videoWriter = cv2.VideoWriter('oto_other.avi', cv2.VideoWriter_fourcc("X", "V", "T", "D"), 30, (1280,720))
    #img_Num = 0
    print("Running...")
    ...
    while key != 113:  # for 'q' key
        ...
        if err == sl.ERROR_CODE.SUCCESS:
            ...
            # # uncomment it to save images
            # path = '/home/kevin/Algorithms/zed_YOLO/zed-yolo3D/zed_python_sample/liveDataSet/'
            # saveImgpath = path + str(img_Num) + '.png'
            # img_Num = img_Num+1
            # #cv2.waitKey(1500)
            # print("done")

            cam.retrieve_measure(
                point_cloud_mat, sl.MEASURE.MEASURE_XYZRGBA)   # get point cloud
            depth = point_cloud_mat.get_data()
            # print(point_cloud_mat.get_infos())
            # depth should be the point cloud data, if we want to pass the data to open3D, probably we can use depth to do or with the help of numpy
            # print([depth])
            pcd = np.random.rand(720*1280,3)
            for i in range(0,720):
                for j in range(0,1280):
                    pcd = point_cloud_mat.get_value(i,j)




            # pcd = point_cloud_mat.get_value()


            # plot the retrived geometry of point cloud
            # draw_geometries(point_cloud_mat, width= 1280, height=720)
            # draw_geometries(np.asarray(depth))

            # bounds = [122, 345, 23]
            # x, y, z = getObjectDepth(depth, bounds)
            # pcd= getObjectDepth(depth, bounds)
            # # mypcd = read_point_cloud(pcd, 'xyz')
            # print(point_cloud_mat.get_resolution())
            pcd.points = Vector3dVector(pcd)
